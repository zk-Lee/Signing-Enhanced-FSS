use crate::{
    ec_cycle_pcd::data_structures::{ECCyclePCDPK, ECCyclePCDVK, HelpCircuit, MainCircuit, Terminal_MainCircuit, Terminal_ECCyclePCDPK, Terminal_ECCyclePCDVK},
    Error, PCDPredicate, PCD, Terminal_PCDPredicate, Terminal_PCD,
};
...
pub struct Terminal_ECCyclePCD<
    CC: TECurveConfig,
    WINDOW: pedersen::Window,
    MainEngine: Pairing,
    HelpEngine: Pairing,
...
impl<CC: TECurveConfig<BaseField = MainEngine::ScalarField>, WINDOW: pedersen::Window, MainEngine: Pairing<ScalarField = BasePrimeField<HelpEngine>>, HelpEngine: Pairing<ScalarField = BasePrimeField<MainEngine>>, IC: ECCyclePCDConfig<MainEngine, HelpEngine>>
    Terminal_PCD<MainEngine::ScalarField, CC, WINDOW, MainEngine, HelpEngine, IC> for Terminal_ECCyclePCD<CC, WINDOW, MainEngine, HelpEngine, IC> {
    type ProvingKey = Terminal_ECCyclePCDPK<CC, WINDOW, MainEngine, HelpEngine, IC>;
    type VerifyingKey = Terminal_ECCyclePCDVK<MainEngine, HelpEngine, IC>;
    type MainProof = <IC::MainSNARK as SNARK<MainEngine::ScalarField>>::Proof;
    type HelpProof = <IC::HelpSNARK as SNARK<HelpEngine::ScalarField>>::Proof;


    fn circuit_specific_setup<P: Terminal_PCDPredicate<MainEngine::ScalarField>, R: Rng + CryptoRng>(
        predicate: &P,
        crh_pp: <CRH<CC, WINDOW> as CRHScheme>::Parameters,
        help_vk: <IC::HelpSNARK as SNARK<HelpEngine::ScalarField>>::VerifyingKey,
        msg: &[P::Message],
        prior_msgs: &[&[P::Message]],
        ...
        let main_circuit = Terminal_MainCircuit::<CC, WINDOW, MainEngine, HelpEngine, IC, P> {
            crh_pp: crh_pp.clone(),
            predicate: predicate.clone(),
            help_vk: Some(help_vk.clone()),
            msg: msg.iter().map(|m| Some(m.clone())).collect(),//vec![None; P::MSG_LEN],
            prior_msgs: prior_msgs.iter().map(|pm| pm.to_vec()).collect(),//Vec::new(),
            prior_proofs: prior_proofs.to_vec(),//Vec::new(),
        };
        let (main_pk, main_vk) = IC::MainSNARK::circuit_specific_setup(main_circuit, rng)?;

        let pk = Terminal_ECCyclePCDPK::<CC, WINDOW, MainEngine, HelpEngine, IC> {
            crh_pp: crh_pp.clone(),
            main_pk,
            help_vk: help_vk.clone(),
        };
        let vk = Terminal_ECCyclePCDVK::<MainEngine, HelpEngine, IC> { main_pvk: IC::MainSNARK::process_vk(&main_vk).unwrap() };

        Ok((pk, vk))
    }

    fn prove<P: Terminal_PCDPredicate<MainEngine::ScalarField>, R: Rng + CryptoRng>(
        pk: &Terminal_ECCyclePCDPK<CC, WINDOW, MainEngine, HelpEngine, IC>,
        predicate: &P,
        msg: &[P::Message],
        prior_msgs: &[&[P::Message]],
        prior_proofs: &[Self::HelpProof],
        ...
        let main_circuit = Terminal_MainCircuit::<CC, WINDOW, MainEngine, HelpEngine, IC, P> {
            ...
            predicate: (*predicate).clone(),
            help_vk: Some(pk.help_vk.clone()),
            msg: msg.iter().map(|m| Some(m.clone())).collect(),
            prior_msgs: prior_msgs.iter().map(|pm| pm.to_vec()).collect(),
            prior_proofs: prior_proofs.to_vec(),
        };
    ...
    fn verify<P: Terminal_PCDPredicate<MainEngine::ScalarField>>(
        vk: &Self::VerifyingKey,
        //msg: &[P::Message],   ///////////////WHY?!?!?!?!?!   GIVE UP, I CANNOT FIX        //MAYBE THE FUNCTION "verify_with_processed_vk" force it
        msg: &[MainEngine::ScalarField],
        proof: &Self::MainProof,
    ) -> Result<bool, Error> {
        let verify_result = IC::MainSNARK::verify_with_processed_vk(&vk.main_pvk, msg, &proof);
