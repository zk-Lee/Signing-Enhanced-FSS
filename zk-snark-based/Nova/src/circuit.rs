...
impl<'a, E: Engine, SC: StepCircuit<E::Base>> NovaAugmentedCircuit<'a, E, SC> {
  ...
  pub fn synthesize<CS: ConstraintSystem<<E as Engine>::Base>>(
    ...
    let z_input = conditionally_select_vec(
      ...
      &z_0,
      &z_i,
      &Boolean::from(is_base_case.clone()), //It was not clone ...(There is no disadvantage if used in a step function.)
    )?;

    let z_next = self
      .step_circuit
      .synthesize(&mut cs.namespace(|| "F"), &z_input, Some(&is_base_case))?;
