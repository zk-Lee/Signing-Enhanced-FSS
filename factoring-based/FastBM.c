...
int setup()
{
    ...
    for(int i=0; i<l; i++) {
        BN_rand_range(s, t1);
        BN_mod_exp(gPriKey->S[i], gPriKey->g, s, gPubKey->N, ctx);
        BN_mod_exp(gPubKey->U[i], gPriKey->S[i], x, gPubKey->N ,ctx);
    }
...
void signMessage(struct FwSecSigSign* sign, BIGNUM* m)
{
	...
	BN_mod_exp(sign->Y, gPriKey->X, e, gPubKey->N, ctx); 
...
int verify(struct FwSecSigSign* sign, BIGNUM** m, struct FwSecSigPubKey* pubKey)
{
	...
	// Z^{2^{(T+1)}}
	exp_twos_power(Z, sign->Z, pubKey->T+1);
...
int verify2(struct FwSecSigSign* sign, BIGNUM** m, struct FwSecSigPubKey* pubKey)
{
	BIGNUM* Z = BN_new();
	BIGNUM* c = BN_new();
	BIGNUM* t1 = BN_new();
	...
	// Z^{2^{(T+1)}}
	exp_twos_power(Z, sign->Z, pubKey->T+1-sign->i);
...
//	for(i=0; i<gPubKey->T; i++) {
	...
	start = clock();
	for(i=0; i<1; i++) {
		if(!verify2(sign[i], &message[i], gPubKey)) {
			printf("sign[%d] is invalid\n", i);
		}
	}
	end = clock();
	printf("Verification2: %f\n", (float)(end-start)/CLOCKS_PER_SEC);
/*
        start = clock();
...
//	printf("%d\n", BN_num_bytes(sign[0]->Z)+BN_num_bytes(sign[0]->Y)+sizeof(int));
    ...
    printf("Pub size: %d\n", x);
    printf("Pri size: %d\n", y);
    printf("sign size: %d\n", BN_num_bytes(sign[0]->Y)+BN_num_bytes(sign[0]->Z)+sizeof(int));
}
