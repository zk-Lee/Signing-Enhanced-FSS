...
void initializeStructures()
{
	...
	gPriKey->sInternal = malloc(sizeof(BIGNUM*)*gPubKey->depth);

	for(int i=0; i<gPubKey->depth; i++) {
		// private keys
		gPriKey->sInternal[i] = BN_new();
	}
...
//	BN_copy(gPubKey->N,rsa->n);
	...
	for(int i=1; i<gPubKey->T; i++) {
		...
		if(ceil(log2(i)) == log2(i)) {
			BN_copy(gPriKey->sInternal[(int)log2(i)], t2);
		}
	}
...
void update()
{
	...
	char rnd_seed[80];
	for(i=0; i<gPubKey->depth; i++) {	// just for time consuming log2(T)
		// FIXME: the i should be properly changed.
		//sprintf(rnd_seed, "%d:%d",seed, i);
		//RAND_seed(rnd_seed, sizeof(rnd_seed));
		//BN_generate_prime_ex(e, l, 0, NULL, NULL, NULL);
		generateE(e, i);
		BN_mod_exp(gPriKey->sInternal[i], gPriKey->sInternal[i], e, gPubKey->N, ctx);
	} 
...
//	file = fopen("output.txt","w");
	...
	gPubKey->depth = (int) ceil(log2(gNumPeriod));
...
//			printf("sign[%d] is invalid\n", i);
    ...
    for( int i = 0; i < gPubKey->depth; i++ ) {
        y += BN_num_bytes(gPriKey->sInternal[i]);
    }
